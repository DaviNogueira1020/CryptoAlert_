...
class AlertsService {
    ...
    async listarAlertas(userId, page = 1, limit = 10, includeUser = false, filters) {
        ...
        if (filters?.crypto)
            where.crypto = filters.crypto.toLowerCase();
    ...
    async obterAlerta(id, userId) {
        return prisma.alert.findFirst({
            where: { id, userId },
            include: { user: { select: { id: true, name: true, email: true } } },
        });
    }
    async atualizarAlerta(id, userId, data) {
        ...
        const updateData = {};
        if (data.crypto)
            updateData.crypto = data.crypto;
        if (data.tipo)
            updateData.tipo = data.tipo;
        if (data.precoAlvo !== undefined)
            updateData.precoAlvo = data.precoAlvo;
        if (data.percentualAlta !== undefined)
            updateData.percentualAlta = data.percentualAlta;
        if (data.percentualQueda !== undefined)
            updateData.percentualQueda = data.percentualQueda;
        if (data.volumeMinimo !== undefined)
            updateData.volumeMinimo = data.volumeMinimo;
        if (data.direction)
        ...
        if (data.alertDate)
            updateData.alertDate = new Date(data.alertDate);
        if (data.alertTime)
            updateData.alertTime = data.alertTime;
        if (data.notificationType)
            updateData.notificationType = data.notificationType;
        if (data.priority)
            updateData.priority = data.priority;
        if (data.repetition)
            updateData.repetition = data.repetition;
        return prisma.alert.update({
            where: { id },
            data: updateData,
        });
    }
    async removerAlerta(id, userId) {
        const alerta = await prisma.alert.findFirst({
            where: { id, userId },
        });
        if (!alerta) {
    ...
    async ativarDesativarAlerta(id, userId, ativo) {
        const alerta = await prisma.alert.findFirst({
            where: { id, userId },
        });
        if (!alerta) {
    ...
    async duplicarAlerta(id, userId) {
        const alerta = await prisma.alert.findFirst({
            where: { id, userId },
        });
        if (!alerta) {
    ...
    async exportarAlertas(userId, formato = 'json') {
    ...
    getCondicaoTexto(alerta) {
        if (alerta.tipo === 'precoAlvo')
            return `$${alerta.precoAlvo} ${alerta.direction === 'above' ? '↑' : '↓'}`;
        if (alerta.tipo === 'altaPercentual')
            return `+${alerta.percentualAlta}%`;
        if (alerta.tipo === 'quedaPercentual')
            return `-${alerta.percentualQueda}%`;
        if (alerta.tipo === 'volume')
            return `Vol: $${alerta.volumeMinimo}`;
        return '—';
    }
}
exports.AlertsService = AlertsService;
//# sourceMappingURL=alerts-new.service.js.map